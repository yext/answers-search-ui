...
describe('GenerativeDirectAnswerComponent renders properly', () => {
  ...
  it('displays the default AI signpost in the answer header', () => {
    ...
    const title = wrapper.find('.yxt-GenerativeDirectAnswer-title').getDOMNode();
  ...
  it('toggles the default AI signpost popover', () => {
    const component = COMPONENT_MANAGER.create(GenerativeDirectAnswerComponent.type, defaultConfig);
    const storage = COMPONENT_MANAGER.core.storage;
    storage.set(StorageKeys.GENERATIVE_DIRECT_ANSWER, gdaSuccess);
    const wrapper = mount(component);
    const button = wrapper.find('.yxt-GenerativeDirectAnswer-aiSignpostButton');
    const popover = wrapper.find('.yxt-GenerativeDirectAnswer-aiSignpostPopover');
    ...
    expect(wrapper.find('.yxt-GenerativeDirectAnswer-aiSignpostPopoverHeaderText').text().trim())
      .toEqual('AI-Generated Content');
    expect(wrapper.find('.yxt-GenerativeDirectAnswer-aiSignpostPopoverBody').text().trim())
      .toEqual('This answer was generated by AI from matching search results and may be incomplete or inaccurate. Review the matching results for full context.');
  ...
  it('hides the AI signpost', () => {
    const component = COMPONENT_MANAGER.create(GenerativeDirectAnswerComponent.type, {
      ...defaultConfig,
      hideAISignpost: true
    });
    const storage = COMPONENT_MANAGER.core.storage;
    ...
    expect(wrapper.find('.yxt-GenerativeDirectAnswer-aiSignpost').exists()).toBeFalsy();
  });

  it('overrides the AI signpost content', () => {
    const component = COMPONENT_MANAGER.create(GenerativeDirectAnswerComponent.type, {
      ...defaultConfig,
      aiSignpost: {
        iconName: 'info',
