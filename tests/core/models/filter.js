...
describe('chec2ing simple filter equality', () => {
  const tomFilter = new Filter({ name: { '$eq': 'Tom' } });
  const alexisFilter = new Filter({ name: { '$eq': 'Alexis' } });

  it('works for equivalent simple filters', () => {
    const tomClone = new Filter({ name: { '$eq': 'Tom' } });
    expect(Filter.areEqualSimpleFilters(tomFilter, tomClone)).toBeTruthy();
    expect(Filter.areEqualSimpleFilters(tomClone, tomFilter)).toBeTruthy();
  });

  it('returns false for different simple filters', () => {
    expect(Filter.areEqualSimpleFilters(tomFilter, alexisFilter)).toBeFalsy();
  });
  ...
  it('does not care about order within filter object', () => {
    ...
    const rangeFilterReverse = new Filter({
      fieldId: {
        '$le': 7,
        '$ge': 5
      }
    });
    expect(Filter.areEqualSimpleFilters(rangeFilter, rangeFilterReverse));
  });
