...
describe('vertical searching for codelab', () => {
  const mockedRequest = jest.fn(() => Promise.resolve({ json: () => Promise.resolve({ test: 'value' }) }));
  let searchApi;
  ...
  it('searches with sara full name and no additional text', () => {
    const result = searchApi.saraSearch(true, false, '');
    expect.assertions(1);
    result.then(results => {
      expect(mockedRequest).toBeCalledWith(
        expect.anything(),
        expect.objectContaining({ input: 'Sara Wong' }));
    });
  });

  it('searches with sara username and additional text', () => {
    const result = searchApi.saraSearch(false, true, 'additional');
    expect.assertions(1);
    result.then(results => {
      expect(mockedRequest).toBeCalledWith(
        expect.anything(),
        expect.objectContaining({ input: 'sawong additional' }));
    });
  });
});
